...
<head>
  ...
  <title>Atomic Habits — Offline Habit Builder</title>
  <style>
    :root{
      --bg:#0B1220;
      --panel:#111B2E;
...
.inline > .field.compact2{ flex: 1 1 170px; min-width: 0; }
    ...
    details.habit:first-of-type{
      border-top: none;
    }
    details.habit > summary{
  list-style:none;
  cursor:pointer;
  padding: 14px;
  display:flex;
  flex-direction:column;         /* <— allows the bar to span full width */
  align-items:stretch;
  justify-content:flex-start;
  gap:10px;                      /* space between top row and bar */
  min-height: var(--tap);
  user-select:none;
  -webkit-tap-highlight-color: transparent;
...
/* Top row inside summary: left content + right action button */
.sumRowTop{
  display:flex;
  align-items:flex-start;
  justify-content:space-between;
...
details.habit > summary::-webkit-details-marker{ display:none; }
details.habit > summary::marker{ content:""; } /* Firefox */

details.habit[open]{
  /* subtle “selected card” ring without changing layout */
  box-shadow: inset 0 0 0 1px rgba(122,167,255,.22);
  background: rgba(255,255,255,.01);
}

details.habit[open] > summary{
  /* accent + clarity, but doesn’t shift the Done button or bar */
  background:
    linear-gradient(90deg, rgba(122,167,255,.18), rgba(255,255,255,.04) 32%),
    rgba(255,255,255,.04);
  border-bottom:1px solid var(--line);
}

details.habit[open] .detailsBody{
  /* make expanded content feel separate from the “main box” */
...
<body>
  <div class="app">
    <header>
      <div class="topbar">
        <div class="brand">
          <div class="title">Atomic Habits — Offline Builder</div>
          <div class="subtitle">Identity • Design • Tracking • Weekly review</div>
        </div>
  ...
  (() => {
    ...
    function motivationalLine(h){
      ...
      if (!identity) return 'Each small action is a vote for the person you’re becoming.';
    ...
    function normalizeData(raw){
      const base = {
        ...
        habits: [],
        reviews: {},
        ui: { activeTab: 'dashboard', createOpen: false, habitSubtabs: {} }
      };
  ...
  updatedAt: (typeof entry.updatedAt === 'string') ? entry.updatedAt : new Date().toISOString()
    ...
    function saveData({ toast=false, toastMsg=null } = {}){
      ...
      try{
        ...
        showToast('Couldn’t save', 'Storage is full or blocked in this browser. Try exporting a backup.', { icon: '!' });
        return;
      }
      if (toast){
...
function renderChartSVG(h, container, rangeKey){
  ...
  container.innerHTML = svg;
    ...
    function renderDashboard(){
      ...
      const createHtml = `
        <div class="createWrap ${createOpen ? 'open':''}" id="createWrap">
          <div class="createHead">
  <button class="btn primary wide" data-action="toggleCreate" aria-expanded="${createOpen ? 'true':'false'}">
    + Create Habit
  </button>
</div>

          <div class="createBody" aria-label="Create Habit Form">
        ...
        <div class="card">
          <div class="cardHeader">
            <div class="left">
              <p class="hTitle">Your habits</p>
              <div class="hMeta">
                <span class="tag muted">${habitCount}/${MAX_HABITS} used</span>
                <span class="muted2">Tap a habit to expand.</span>
              </div>
            </div>
            <div class="right">
              <button class="btn small ghost" data-action="quickBackup" title="Export JSON backup">Export</button>
            </div>
          </div>
    ...
    function renderCreateForm(){
      ...
      return `
        <div class="grid two">
          <div class="field">
            <label for="c_name">Habit name *</label>
            <input id="c_name" type="text" placeholder="e.g., Walk after lunch" autocomplete="off" maxlength="120" />
            <div class="help">Keep it concrete and action-based.</div>
          </div>
          <div class="field">
            <label for="c_type">Habit type *</label>
            <select id="c_type">
              <option value="positive">Positive (build)</option>
              <option value="negative">Negative (reduce)</option>
            </select>
            <div class="help">Negative habits track “Resisted / Relapsed”.</div>
          </div>
        </div>

        <div class="field" style="margin-top:12px;">
          <label for="c_desc">Description *</label>
          <textarea id="c_desc" placeholder="What does success look like? Keep it shame-free." maxlength="1200"></textarea>
        </div>

        <div class="inline freqRow" style="margin-top:12px;">
          <div class="field compact2">
            <label for="c_start">Start date *</label>
            <input id="c_start" type="date" value="${d}" />
          </div>
          <div class="field compact">
            <label for="c_unit">Frequency unit *</label>
<select id="c_unit">
  ...
  <option value="month">Month</option>
          ...
          <div class="field compact">
            <label for="c_target">Target # *</label>
        ...
        <div class="grid two">
          <div class="field">
            <label for="c_ident">Identity statement * <span class="muted2">(required)</span></label>
            <input id="c_ident" type="text" placeholder="I am the kind of person who…" maxlength="220" />
            <div class="help">Example: “I am the kind of person who moves daily.”</div>
          </div>
          <div class="field">
            <label for="c_outcome">Long-term outcome *</label>
            <input id="c_outcome" type="text" placeholder="What does this enable over months/years?" maxlength="260" />
          </div>
        </div>

        <div class="field" style="margin-top:12px;">
          <label for="c_why">Why this matters *</label>
          <textarea id="c_why" placeholder="A calm, honest reason you can return to." maxlength="420"></textarea>
        </div>

        <div id="createErr" style="margin-top:12px;"></div>

        <div class="row" style="margin-top:12px;">
          <button class="btn primary wide" data-action="saveCreate">Save habit</button>
          <button class="btn ghost wide" data-action="cancelCreate">Cancel</button>
        </div>
      `;
    }
...
function renderDashboardQuick(h){
  ...
  return `
    <div class="dashMini" id="dash-${escapeHtml(h.id)}-mini">
...
function updateDashboardQuick(habitId){
  ...
  document
    .querySelectorAll(`button[data-action="dashToggle"][data-habit="${habitId}"]`)
    .forEach((btn) => {
...
function dashToggle(habitId){
  ...
  if (isDone){
    ...
    showToast('Reset', 'Back to zero for today. You can check it off again anytime.', { icon:'↺', ms:1800 });
  } else {
    // One tap = complete the full daily target
    e.status = succ;
    e.count = target;
    e.updatedAt = new Date().toISOString();
    showToast('Nice', motivationalLine(h), { icon:'✓', ms:5000 });
  }
}
    ...
    if (st === succ){
      ...
      e.count = 0;
      e.updatedAt = new Date().toISOString();
      showToast('Undone', 'Your log was updated.', { icon:'↺', ms:1800 });
    } else {
      e.status = succ;
      e.count = 1;
      e.updatedAt = new Date().toISOString();
      showToast('Nice', motivationalLine(h), { icon:'✓', ms:2600 });
    }
  }
...
function renderHabitList(){
  ...
  const items = habits.map(h => {
    ...
    const last12 = actualCountLast12Months(h);

    const subtab = appData.ui.habitSubtabs?.[h.id] || 'track';

    const succStatus = successStatusForType(h.type);
    const failStatus = failStatusForType(h.type);

    return `
      <details class="habit" id="habit-${escapeHtml(h.id)}" data-habit="${escapeHtml(h.id)}">
        <summary>
          <div class="sumRowTop">
            <div class="sumLeft">
              <div class="sumTop">
                <div class="sumName" data-sum="name">${escapeHtml(h.name || 'Untitled habit')}</div>

                <span class="tag ${tagClass}">${typeLabel}</span>
                <span class="tag muted" data-sum="freq">${escapeHtml(freqTxt)}</span>

                <div class="sumDesc" data-sum="desc">${escapeHtml(h.description || '')}</div>
              </div>
            </div>

            <div class="sumRight">
              <button
                class="btn small ${escapeHtml(dashboardState(h).btnClass)} dashBtn"
                data-action="dashToggle"
                data-habit="${escapeHtml(h.id)}"
                aria-label="Mark ${escapeHtml(h.name || 'habit')} for today"
              >${escapeHtml(dashboardState(h).btnLabel)}</button>
            </div>
          </div>
        ...
        <div class="detailsBody">
          <div id="habit-${escapeHtml(h.id)}-summary">
            ${renderHabitSummary(h, { prog, c7, streak, last12 })}
          </div>

          <div class="subtabs" role="tablist" aria-label="Habit sections">
            <button class="tabBtn ${subtab==='track'?'active':''}"
              data-action="habitSubtab" data-habit="${escapeHtml(h.id)}" data-subtab="track"
              role="tab" aria-selected="${subtab==='track'?'true':'false'}">Track</button>

            <button class="tabBtn ${subtab==='design'?'active':''}"
              data-action="habitSubtab" data-habit="${escapeHtml(h.id)}" data-subtab="design"
              role="tab" aria-selected="${subtab==='design'?'true':'false'}">Design</button>

            <button class="tabBtn ${subtab==='insights'?'active':''}"
              data-action="habitSubtab" data-habit="${escapeHtml(h.id)}" data-subtab="insights"
              role="tab" aria-selected="${subtab==='insights'?'true':'false'}">Insights</button>

            <button class="tabBtn ${subtab==='motivation'?'active':''}"
              data-action="habitSubtab" data-habit="${escapeHtml(h.id)}" data-subtab="motivation"
              role="tab" aria-selected="${subtab==='motivation'?'true':'false'}">Motivator</button>

            <button class="tabBtn ${subtab==='edit'?'active':''}"
              data-action="habitSubtab" data-habit="${escapeHtml(h.id)}" data-subtab="edit"
              role="tab" aria-selected="${subtab==='edit'?'true':'false'}">Edit</button>
          </div>

          <div class="panel ${subtab==='track'?'':'hidden'}" id="panel-${escapeHtml(h.id)}-track" role="tabpanel">
            ${renderHabitTrackPanel(h, { succStatus, failStatus })}
          </div>

          <div class="panel ${subtab==='design'?'':'hidden'}" id="panel-${escapeHtml(h.id)}-design" role="tabpanel">
            ${renderHabitDesignPanel(h)}
          </div>

          <div class="panel ${subtab==='insights'?'':'hidden'}" id="panel-${escapeHtml(h.id)}-insights" role="tabpanel">
            ${renderHabitInsightsPanel(h)}
          </div>

          <div class="panel ${subtab==='motivation'?'':'hidden'}" id="panel-${escapeHtml(h.id)}-motivation" role="tabpanel">
            ${renderHabitMotivationPanel(h)}
          </div>

          <div class="panel ${subtab==='edit'?'':'hidden'}" id="panel-${escapeHtml(h.id)}-edit" role="tabpanel">
          ...
          <div class="muted tiny">Tip: Inputs save instantly. “Save” simply confirms and refreshes visuals.</div>
        </div>
      </details>
    `;
  }).join('');
  ...
  return items;
    ...
    function renderHabitSummary(h, pre){
      ...
      const c30 = windowConsistencyPercent(h, 30);
      const c90 = windowConsistencyPercent(h, 90);
      const streak = pre?.streak ?? currentDailyStreak(h);
      const last12 = pre?.last12 ?? actualCountLast12Months(h);
      const freqTxt = formatUnit(h.frequency.unit, h.frequency.target);
      ...
      return `
        <div class="stats" aria-label="Tracking summary">
          <div class="stat">
            <div class="k">Planned</div>
            <div class="v">${escapeHtml(freqTxt)}</div>
          ...
          <div class="stat">
            ...
            <div class="s">${escapeHtml(prog.startISO)}</div>
          </div>
          <div class="stat">
            <div class="k">Consistency</div>
            <div class="v">${escapeHtml(String(c7))}%</div>
            <div class="s">7d • 30d ${escapeHtml(String(c30))}%</div>
          </div>
          <div class="stat">
            <div class="k">Streak</div>
            <div class="v">${escapeHtml(String(streak))}d</div>
            <div class="s">90d ${escapeHtml(String(c90))}% • 12mo ${escapeHtml(String(last12))}</div>
          </div>
        </div>
    ...
    <div class="panel" style="border:none; background:transparent; padding:0;">
      ...
      <div class="field" style="margin-top:12px;">
        ...
        <textarea
          ...
          data-habit="${escapeHtml(h.id)}"
          maxlength="1200"
          placeholder="What does success look like? Keep it shame-free.">${escapeHtml(h.description || '')}</textarea>
      </div>
...
function updateHabitHeader(habitId){
  ...
  const det = document.getElementById(`habit-${habitId}`);
  if (!det) return;

  const nameEl = det.querySelector('[data-sum="name"]');
  if (nameEl) nameEl.textContent = h.name || 'Untitled habit';

  const descEl = det.querySelector('[data-sum="desc"]');
  if (descEl) descEl.textContent = h.description || '';

  const freqEl = det.querySelector('[data-sum="freq"]');
  if (freqEl) freqEl.textContent = formatUnit(h.frequency.unit, h.frequency.target);

  const dashBtn = det.querySelector(`button[data-action="dashToggle"][data-habit="${habitId}"]`);
  if (dashBtn) dashBtn.setAttribute('aria-label', `Mark ${h.name || 'habit'} for today`);
}

    function renderHabitMotivationPanel(h){
      ...
      return `
        ...
        <div class="success" style="margin-top:12px;">
          <strong>How it will be used:</strong>
          <div class="small muted" style="margin-top:6px;">
            This motivator shows up in your tracking feedback, weekly review prompts, and gentle encouragement.
          </div>
        </div>
      `;
    }
...
const plannedInRange = Math.round(plannedRate * w.days);
  ...
  `;
    ...
    function renderHabitTrackPanel(h, { succStatus, failStatus }){
      ...
      const defaultStatus = existing?.status || succStatus;
      ...
      return `
        <div class="panel" style="border:none; background:transparent; padding:0;">
          ...
          <div style="font-weight:950; margin-bottom:8px;">Recent logs</div>
          <div class="muted small" style="margin-bottom:10px;">Edit notes anytime. Status changes update your graph and progress immediately.</div>
...
function renderRecentLogs(h){
  const logs = safeObj(h.logs);
  const keys = Object.keys(logs).filter(k => /^\d{4}-\d{2}-\d{2}$/.test(k));
  ...
  let inner = '';
  if (!keys.length){
    inner = `<div class="panel"><div class="muted small">No logs yet. Start with a single calm entry today.</div></div>`;
  } else {
  ...
  `;
    ...
    function renderReview(){
      ...
      root.innerHTML = `
        <div class="reviewHero">
          ...
          <p class="smallp">
            Calm reflection, no shame. Use what happened as data to improve the system (cues, friction, environment).
          </p>
        </div>
        ...
        <div class="card">
          ...
          <div class="content">
            ...
            <div class="row" style="margin-top:12px;">
              ...
              <button class="btn ghost wide" data-action="copyReviewPrompt">Copy prompts</button>
            </div>

            <div class="muted tiny" style="margin-top:10px;">
              Everything saves instantly, but “Save review” gives you a clear checkpoint.
            </div>
          </div>
        </div>
          ...
          <div class="content">
            <div class="panel">
              ...
              <div class="muted small" style="margin-top:10px;">
                Export regularly if this matters to you. Import supports merge or replace (with validation).
              </div>
            </div>
    ...
    function hydrateVisibleCharts(){
      // Render charts for open habits whose Insights panel is visible OR chart container exists within open details.
      // We render lazily on open, and also when logs change.
      const openDetails = $$('details.habit[open]');
      openDetails.forEach(det => {
        const hid = det.getAttribute('data-habit');
        if (!hid) return;
        const sub = appData.ui.habitSubtabs?.[hid] || 'track';
        if (sub !== 'insights') return;
        const chartEl = $(`#habit-${CSS.escape(hid)}-chart`);
    ...
    function updateHabitVisuals(habitId){
      ...
      const sumWrap = $(`#habit-${CSS.escape(habitId)}-summary`);
      if (sumWrap){
        const prog = periodProgress(h);
        const c7 = windowConsistencyPercent(h, 7);
        const streak = currentDailyStreak(h);
        const last12 = actualCountLast12Months(h);
        sumWrap.innerHTML = renderHabitSummary(h, { prog, c7, streak, last12 });
      }

      // If insights panel visible, update chart
      const sub = appData.ui.habitSubtabs?.[habitId] || 'track';
      if (sub === 'insights'){
        const chartEl = $(`#habit-${CSS.escape(habitId)}-chart`);
...
function ensureDefaultLogsUpToToday(){
  ...
  if (changed) saveData({ toast:false });
    ...
    function validateCreate(){
      ...
      if (!ident) errors.push('Identity statement is required.');
      if (!outcome) errors.push('Long-term outcome is required.');
      if (!why) errors.push('Why this matters is required.');
  ...
  [todayISO()]: {
    ...
    function saveHabit(habitId){
      ...
      if (missing.length){
        ...
        const input = document.querySelector(`[data-habit="${CSS.escape(habitId)}"][data-bind="${selector}"]`);
    ...
    function setHabitSubtab(habitId, subtab){
      ...
      // buttons
      $$(`[data-action="habitSubtab"][data-habit="${CSS.escape(habitId)}"]`, det).forEach(btn => {
      ...
      // panels
      ['track','design','insights','motivation','edit'].forEach(t => {
      ...
      if (subtab === 'insights'){
        ...
        if (h && chartEl){
          renderChartSVG(h, chartEl, getInsightRangeKey(habitId));
        }
      }
    }
    ...
    function saveLog(habitId){
      ...
      const det = document.getElementById(`habit-${habitId}`);
      const dateEl = det?.querySelector(`[data-track="date"][data-habit="${CSS.escape(habitId)}"]`);
      const statusEl = det?.querySelector(`[data-track="status"][data-habit="${CSS.escape(habitId)}"]`);
      const noteEl = det?.querySelector(`[data-track="note"][data-habit="${CSS.escape(habitId)}"]`);
...
h.logs[date] = { date, status, note, count, auto:false, updatedAt: new Date().toISOString() };
    ...
    function clearLogInputs(habitId){
      ...
      if (statusEl){
        const h = getHabit(habitId);
        if (h) statusEl.value = successStatusForType(h.type);
      }
    ...
    function openModal(title, bodyHtml, footHtml=''){
      ...
      setTimeout(() => {
        const focusable = modal.querySelector('textarea, input, button, select, [tabindex]:not([tabindex="-1"])');
    ...
    function openExport(){
      ...
      const body = `
        <div class="muted small" style="margin-bottom:10px;">
          Copy this JSON somewhere safe. (This app runs offline; backups are your safety net.)
        </div>
    ...
    function validateImported(rawText){
      ...
      // required: motivator must be present and filled (per habit)
      for (const h of normalized.habits){
        const m = safeObj(h.motivator);
        if (!String(m.identity||'').trim() || !String(m.outcome||'').trim() || !String(m.why||'').trim()){
          return { ok:false, error:`Import blocked: habit “${h.name || 'Untitled'}” is missing required motivator fields.` };
        }
      }
    ...
    function saveReview(weekISO){
      ...
      saveData({ toast:true, toastMsg:'Weekly review saved.' });
    }
    ...
    function runDiagnostics(){
      try{
        ...
        for (const h of d.habits){
          ...
          const m = safeObj(h.motivator);
          if (!String(m.identity||'').trim() || !String(m.outcome||'').trim() || !String(m.why||'').trim()){
            return { ok:false, summary:`Habit “${h.name}” is missing required motivator fields.` };
          }
          // logs validity
          const logs = safeObj(h.logs);
          for (const k of Object.keys(logs)){
...
if (action === 'setInsightRange'){
  ...
  return;
      ...
      if (action === 'downloadExport'){
        const text = $('#exportText')?.value || '';
        const filename = `atomic-habits-backup-${todayISO()}.json`;
    ...
    document.addEventListener('input', (e) => {
      ...
      if (el.hasAttribute('data-bind') && el.hasAttribute('data-habit')){
        handleBoundInput(el);
        return;
      }
    ...
    document.addEventListener('toggle', (e) => {
      ...
      if (!det.classList.contains('habit')) return;
      if (det.open){
        const hid = det.getAttribute('data-habit');
        if (!hid) return;
        const sub = appData.ui.habitSubtabs?.[hid] || 'track';
        if (sub === 'insights'){
          const h = getHabit(hid);
          const chartEl = document.getElementById(`habit-${hid}-chart`);
          if (h && chartEl) renderChartSVG(h, chartEl, getInsightRangeKey(hid));
        }
      }
    }, true);
